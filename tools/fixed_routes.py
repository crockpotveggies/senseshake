...
def add(b):
    for row in json.loads((ROOT/'layout-fixed-routes.json').read_text()):
        if row['type']=='via':
...
if __name__=='__main__':
    ...
    b=p.LoadBoard(str(ROOT/'hardware/shakesense-hat/shakesense-hat.kicad_pcb'));rows=[]
    for t in b.GetTracks():
        ...
        rows.append(row)
    (ROOT/'layout-fixed-routes.json').write_text(json.dumps(rows,indent=2));print('Stored',len(rows),'fixed connector routing objects')
