...
def main():
    metadata=json.loads((ROOT/'layout.json').read_text());source=(ROOT/'elec/parts.ato').read_text()
    blocks={m[1]:m[2] for m in re.finditer(r'^component (\w+):\n(.*?)(?=^component |\Z)',source,re.M|re.S)}
    reports=[]
    for name,meta in metadata.items():
        target='hat' if name.endswith('-hat') else 'field_head';b=p.LoadBoard(str(ROOT/'layout'/target/(target+'.kicad_pcb')))
    ...
    (ROOT/'simulation/circuit-checks.json').write_text(json.dumps(result,indent=2));print(json.dumps(result,indent=2))
