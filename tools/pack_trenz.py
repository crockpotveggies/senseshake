...
import json,math
import pcbnew as p
ROOT=Path(__file__).resolve().parents[1];path=ROOT/'layout-trenz.json';data=json.loads(path.read_text());spec=data['shakesense-trenz-hat']
...
def bounds(m,xy=None,angle=None):
    f=p.FootprintLoad(str(ROOT/'elec'),m['local_fp']);x,y=xy or m['xy'];f.SetPosition(p.VECTOR2I(p.FromMM(x),p.FromMM(y)));f.SetOrientationDegrees(m['angle'] if angle is None else angle)
