...
for name in ['shakesense-hat','shakesense-field-head']:
    r=json.loads((ROOT/'hardware'/name/'validation.json').read_text())
...
sim=json.loads((ROOT/'simulation/results.json').read_text());assert len(sim['cases'])==27 and all(not x['failures'] for x in sim['cases'])
cir=json.loads((ROOT/'simulation/circuit-checks.json').read_text());assert all(x['circuit_invariants']=='PASS' for x in cir['boards'])
pos=(ROOT/'logs/constraint-solve.log').read_text();neg=(ROOT/'logs/negative-voltage.log').read_text()
...
files=list((ROOT/'elec').glob('*.ato'))+[ROOT/'ato.yaml',ROOT/'layout.json',ROOT/'layout-fixed-routes.json']
files.extend((ROOT/'tools').glob('*.py'))
files.extend((ROOT/'simulation').glob('*.cir'))
for name in ['shakesense-hat','shakesense-field-head']:
...
files.extend([ROOT/'simulation/results.json',ROOT/'simulation/circuit-checks.json'])
...
`schematic-netlist.xml` under `hardware/`. `tools/check_design.py` checks source,
...
- `tools/check_circuit.py`: compiled pin assignments, source/BOM agreement,
...
- **27 ngspice cases pass**: rail/load/USB cable corners, I2C rise time, UART RC,
  ...
  See [simulation scope](../simulation/README.md) and the actual decks/logs.
