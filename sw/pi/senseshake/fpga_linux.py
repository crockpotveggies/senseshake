...
class Lines:
    def __init__(self,chip,offsets,values=None):
        ...
        try:
            ...
            req.consumer=b'senseshake-fpga'
...
class Owner:
    ...
    def spi_path(self):
        ...
        device=paths[0]
        node=device/'of_node'
        if b'senseshake,fpga-userspace' not in (node/'compatible').read_bytes().split(b'\0'):
            raise RuntimeError('SPI6 device is not the T1-LINK overlay')
