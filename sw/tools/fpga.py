...
from groundlark.fpga_link import ModeControl,Packets
...
def main():
    ...
    parser.add_argument('--message',default='Groundlark internal SPI link')
    ...
    import fcntl
    from groundlark.fpga_linux import Arm,Owner
    from groundlark.linux_io import I2C,SPI
    # Root-owned directory prevents an unprivileged user supplying a lock symlink.
    lockdir=Path('/run/groundlark-fpga');lockdir.mkdir(mode=0o700,exist_ok=True)
    with (lockdir/'owner.lock').open('a') as lock:
        ...
        try:
            ...
            with control.active(mode):
                if mode=='jtag':
                    cfg=Path(__file__).resolve().parents[1]/'pi/deploy/daqhat-01-jtag.cfg'
